...
import { getVaultConfig, VaultConfig, VaultKubernetesAuthConfig} from '../config';
...
/**
 * Object received as a response from the Vault API when fetching secrets
 * Deprecated, use VaultSecretData instead
 * @internal
 */
  ...
  async listSecrets(
    ...
    const listUrl =
      this.vaultConfig.kvVersion === 2
        ? `v1/${encodeURIComponent(mount)}/metadata/`
        : `v1/${encodeURIComponent(mount)}/data/${secretPath}`;

    const result = await this.limit(() =>
      this.callApi<VaultSecretData>(listUrl, {}),
    );

    const secrets: VaultSecret[] = [];
    let modifiedKeys = Object.keys(result.data.data)
    await Promise.all(
      modifiedKeys.map(async secret => {
        if (result.data.data[secret].endsWith('/')) {
          secrets.push(
            ...(await this.limit(() =>
